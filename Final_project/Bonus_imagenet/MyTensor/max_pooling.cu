...
__global__ void max_pool_backward(
    float* out_grad, float* out_mask, float* in_grad, int nthreads, int num, int in_channels, int in_h, int in_w, int out_h, int out_w) {
    
    CUDA_KERNEL_LOOP(index, nthreads) {
        int n = index / (out_h * out_w * in_channels); // Batch index
        int c = (index / (out_h * out_w)) % in_channels; // Channel index
        int ph = (index / out_w) % out_h; // Output height index
        int pw = index % out_w; // Output width index

        // Get the index of the max value in the input tensor
        int idx = out_mask[n * in_channels * out_h * out_w + c * out_h * out_w + ph * out_w + pw];

        // Update the gradient of the input tensor
        in_grad[n * in_channels * in_h * in_w + c * in_h * in_w + idx] = out_grad[n * in_channels * out_h * out_w + c * out_h * out_w + ph * out_w + pw];
    }
}
...
void max_pooling_backward(const Tensor& out_grad, Tensor& out_mask, Tensor& in_grad) {
    // Get the dimensions of the input tensor
    int num = in_grad.shape[0];
    int in_channels = in_grad.shape[1];
    int in_h = in_grad.shape[2];
    int in_w = in_grad.shape[3];

    // Get the dimensions of the output tensor
    int out_h = out_grad.shape[2];
    int out_w = out_grad.shape[3];

    // Calculate the number of threads
    int nthreads = num * in_channels * out_h * out_w;

    // Call the CUDA kernel
    int num_blocks = CudaGetBlocks(nthreads);
    max_pool_backward<<<num_blocks, kCudaThreadsNum>>>(
        out_grad.data, out_mask.data, in_grad.data, nthreads, num, in_channels, in_h, in_w, out_h, out_w);
}
