"""
利用作业五中的Adam_epoch优化器，和作业四实现的自动微分。在网络结构中加入卷积层，在set_structure函数中增加kernel权重，相应修改forward函数，在Adam_epoch中增加更新kernel权重的代码。在此基础上，我实现了更一般地权重优化方法，以避免每次更改模型架构都需要相应更改优化器。
"""
...
def set_structure(n, hidden_dim, k, model):
    ...
    if model == "LeNet":
        # 实现LeNet结构
        W_conv1 = np.random.randn(6, 1, 5, 5).astype(np.float32) / 5 # [Cout, Cin, kH, kW]
        W_conv2 = np.random.randn(16, 6, 5, 5).astype(np.float32) / 5 
        W1 = np.random.randn(16 * 4 * 4, 120).astype(np.float32) / np.sqrt(16 * 4 * 4)
        W2 = np.random.randn(120, 84).astype(np.float32) / np.sqrt(120)
        W3 = np.random.randn(84, 10).astype(np.float32) / np.sqrt(84)
...
def forward(X, weights):
    ...
    if model == "LeNet":
        ...
        X = max_pooling(relu((conv2d(w_conv1, X, 0, 1))))
        X = max_pooling(relu((conv2d(w_conv2, X, 0, 1))))
...
def opti_epoch(X, y, weights, lr = 0.1, batch=100, beta1=0.9, beta2=0.999):
    """
    优化一个epoch
...
def train_nn(X_tr, y_tr, X_te, y_te, hidden_dim = 500,
    ...
    for epoch in range(epochs):
        ...
        opti_epoch(X_tr, y_tr, weights_tensor, lr=lr, batch=batch, beta1=beta1, beta2=beta2)
        train_loss, train_err = loss_err(forward(Tensor(X_tr), weights_tensor), y_tr)
        test_loss, test_err = loss_err(forward(Tensor(X_te), weights_tensor), y_te)
...
if __name__ == "__main__":
    ...
    train_nn(X_tr, y_tr, X_te, y_te, hidden_dim=100, epochs=20, lr = 0.001, batch=100, beta1=0.9, beta2=0.999, model = model)
